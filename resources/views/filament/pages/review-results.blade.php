<x-filament-panels::page>
    ...
    @if (! $review_window_id)
        <div class="p-12 text-center bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs">
    ...
    @else
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            @forelse ($this->sections as $section)
                ...
                <div class="group bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs hover:shadow-md transition-all duration-200 p-6 flex flex-col justify-between">
                    <div>
                        {{-- Section Header --}}
                        <div class="flex items-start justify-between border-b border-slate-100 dark:border-slate-800/80 pb-4 mb-4">
                            <div class="space-y-1">
                                <div class="flex items-center gap-2">
                                    <span class="px-2.5 py-0.5 rounded-md text-[11px] font-extrabold uppercase tracking-wider bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300 border border-blue-200/60 dark:border-blue-800/60">
                                        {{ $section->course?->code }}
                                    </span>
                            ...
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold shrink-0 {{ $result['response_count'] >= 5 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300' }}">
                        ...
                        <div class="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800/50 mb-5 border border-slate-100 dark:border-slate-800">
                            <div class="flex items-center justify-center w-7 h-7 rounded-lg bg-gradient-to-br from-blue-700 to-indigo-800 text-white font-bold text-xs">
                        ...
                        @if ($result['is_suppressed'])
                            <div class="rounded-xl bg-amber-50/70 dark:bg-amber-950/30 border border-dashed border-amber-300 dark:border-amber-700/60 p-5 text-center my-2">
                                ...
                                @foreach ($result['questions_data'] as $q)
                                    <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800">
                                        <div class="flex items-center justify-between text-xs font-semibold text-slate-700 dark:text-slate-200 mb-1.5">
                                            ...
                                            @if ($q['type'] === 'rating')
                                                <span class="font-bold text-blue-700 dark:text-blue-300 bg-blue-100 dark:bg-blue-950/70 px-2 py-0.5 rounded text-[11px] shrink-0">
                                                    {{ $q['average'] }} / 5.0 ⭐
                                                </span>
                                            @elseif ($q['type'] === 'yes_no')
                                                <span class="font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-950/70 px-2 py-0.5 rounded text-[11px] shrink-0">
                                        ...
                                        @if ($q['type'] === 'rating')
                                            <div class="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2 overflow-hidden">
                                                <div class="bg-gradient-to-r from-blue-700 to-indigo-600 h-2 rounded-full transition-all duration-500" style="width: {{ $q['percentage'] }}%"></div>
                                            </div>
                                        @elseif ($q['type'] === 'yes_no')
                                            <div class="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2 overflow-hidden">
                                                <div class="bg-gradient-to-r from-emerald-600 to-teal-500 h-2 rounded-full transition-all duration-500" style="width: {{ $q['percentage_yes'] }}%"></div>
                                            </div>
                                        @endif
                                    </div>
                                @endforeach
