...
class ReviewQuestionResource extends Resource
{
    ...
    protected static ?string $modelLabel = 'Review Question';

    protected static ?string $pluralModelLabel = 'Review Questions';
    ...
    public static function getEloquentQuery(): Builder
    {
        return parent::getEloquentQuery()
            ->where('form_type', FormType::StudentReview);
    }
                ...
                Forms\Components\Card::make()
                    ->schema([
                        Forms\Components\Hidden::make('form_type')
                            ->default(FormType::StudentReview->value),
        ...
        return $table
            ...
            ->actions([
                ...
                Tables\Actions\DeleteAction::make()
                    ->before(function (Tables\Actions\DeleteAction $action, Question $record) {
                        $hasActiveWindow = \App\Models\ReviewWindow::where('status', \App\Enums\ReviewWindowStatus::Active)
                            ->where('starts_at', '<=', now())
                            ->where('ends_at', '>=', now())
                            ->exists();
                        ...
                        if ($hasActiveWindow) {
                            \Filament\Notifications\Notification::make()
                                ...
                                ->body('Cannot delete review questions while a review window is actively open.')
                                ->send();
                            $action->halt();
                        }

                        $hasResponses = \App\Models\ReviewResponse::whereNotNull("answers_json->{$record->id}")->exists();
                        if ($hasResponses) {
                            \Filament\Notifications\Notification::make()
                                ->danger()
                                ->title('Cannot Delete Question')
                                ->body('This question has existing recorded student review responses. Please deactivate the question instead.')
                                ->send();
                            $action->halt();
                        }
                    }),
            ...
            ->emptyStateHeading('No review questions yet')
            ->emptyStateDescription('Create evaluation questions for student course reviews.')
